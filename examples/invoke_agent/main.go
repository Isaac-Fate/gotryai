// This example demonstrates a minimal agent with no tools.
//
// CreateAgentMap builds an agent loop (agent node → tool node → agent node).
// With empty inputTools, the agent only does LLM calls—no tool execution.
//
